...
libvirt-lxc)
	cat <<EOF
#$set_color_red WARNING: make sure root stays uid-mapped, otherwise you might experience store corruption in the host!$reset_color
# Create $name using:
virt-install \\
  --connect lxc:///system \\
  -n $name --os-variant nixos-unstable \\
  --memory 1536 \\
  --import \\
  --idmap uid.start=0,uid.target=1000000,uid.count=10 \\
  --idmap gid.start=0,gid.target=1000000,gid.count=10 \\
  --boot init=/boot/init \\
  --filesystem /nix/store,/nix/store,readonly=yes \\
  --filesystem /etc/miniguests/$name/boot,/boot,readonly=yes \\
